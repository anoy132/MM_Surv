...
class Transformer_MAE(nn.Module):
    def __init__(
        self,
        dim,
        *,
    ...
    def forward(
        ...
        x_masked = None
        if mae_kwargs is not None:
            assert not self.casual
            ids_keep = mae_kwargs['ids_keep']   # [b,keep_num]
            x_masked = torch.gather(input_x, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, input_x.shape[-1]))
            n,keep_num,d = x_masked.shape
            
            if attn_bias is not None:
                assert attn_bias.shape[1] == attn_bias.shape[2]
                n_heads, original_l, _ = attn_bias.shape
                ids_keep_row_expanded = ids_keep.unsqueeze(1).unsqueeze(-1).expand(-1, n_heads, keep_num, original_l)
                attn_bias = attn_bias.unsqueeze(0).expand(n, -1, -1, -1) 
                kept_rows = torch.gather(attn_bias, dim=2, index=ids_keep_row_expanded)
                ids_keep_col_expanded = ids_keep.unsqueeze(1).unsqueeze(-2).expand(-1, n_heads, keep_num, keep_num)
                attn_bias = torch.gather(kept_rows, dim=3, index=ids_keep_col_expanded)

            for peg, self_attn, cross_attn, ff in self.layers:
                if exists(peg):
                    temp = peg(input_x, shape = video_shape)
                    ids_keep_temp_expand = ids_keep.unsqueeze(dim=-1).expand(-1,-1,temp.shape[-1])
                    temp = torch.gather(temp, 1, index=ids_keep_temp_expand)
                    x_masked = temp + x_masked

                assert self_attn_mask is None
                x_masked = self_attn(x_masked, attn_bias = attn_bias, mask = self_attn_mask) + x_masked

                assert cross_attn is None
                assert context is None

                x_masked = ff(x_masked) + x_masked

            x_masked = self.norm_out(x_masked)

        return_dict = {
            'intact_output': x,
            'masked_output': x_masked
        }
